                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-[var(--bg-surface-elevated)]">
                                <tr>
                                    <th className="px-4 py-3 text-[10px] font-black text-[var(--text-muted)] uppercase">Tenant</th>
                                    <th className="px-4 py-3 text-[10px] font-black text-[var(--text-muted)] uppercase text-right">Available</th>
                                    <th className="px-4 py-3 text-[10px] font-black text-[var(--text-muted)] uppercase text-right">Pending</th>
                                    <th className="px-4 py-3 text-[10px] font-black text-[var(--text-muted)] uppercase text-right">Settled</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-[var(--border-subtle)]">
                                {wallets.map((w) => (
                                    <tr key={w.id} className="hover:bg-[var(--bg-surface-elevated)] transition-colors">
                                        <td className="px-4 py-4 font-bold text-[var(--text-primary)]">{w.tenantName}</td>
                                        <td className="px-4 py-4 font-black text-right text-emerald-500">KES {Number(w.balance).toLocaleString()}</td>
                                        <td className="px-4 py-4 font-black text-right text-amber-500">KES {Number(w.pendingBalance).toLocaleString()}</td>
                                        <td className="px-4 py-4 font-black text-right text-sky-500">KES {Number(w.settledBalance).toLocaleString()}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>

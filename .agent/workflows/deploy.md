...
Follow these steps to deploy the application to the production server (`129.212.228.90`).
...
// turbo
```powershell
scp -r frontend/dist/* root@129.212.228.90:/var/www/app
```
...
// turbo
```powershell
scp -r src/* root@129.212.228.90:/root/Billing-System-/src/
```

## 4. Restart Backend via PM2
SSH into the server and restart the backend process.
```powershell
ssh root@129.212.228.90 "pm2 restart billing-system || pm2 start /root/Billing-System-/src/server.ts --interpreter ts-node --name billing-system && pm2 save"
```

## 5. Verify Deployment
Test the API and frontend accessibility.
```powershell
curl http://129.212.228.90/api/test
```
